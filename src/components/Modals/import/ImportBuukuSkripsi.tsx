...
import { sampleTASkripsiExport } from "@/services/karya-tulis/http";
import { useImportTASkripsi } from "@/services/karya-tulis";
...
const ImportBukuSkripsi = ({ isOpen, onClose, confirmAction }: Props) => {
  ...
  const {mutate: mutatePost} = useImportTASkripsi();
  const handleSubmit = () => {
    setLoading(true)
    try {
      mutatePost(
...
export default ImportBukuSkripsi;
