...
import { sampleBukuDigitalExport } from "@/services/buku-digital/http";
import { useImportBukuDigital } from "@/services/buku-digital";
...
const ImportBukuDigital = ({ isOpen, onClose, confirmAction }: Props) => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      await sampleBukuDigitalExport();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (error: { status: number } | any) {
      if (error?.status === 404) {
        errorToast({
          text: "Data buku digital tidak ditemukan",
        });
      }
      throw error;
    } finally {
  ...
  const {mutate: mutatePost} = useImportBukuDigital();
  const handleSubmit = () => {
    setLoading(true)
    try {
      mutatePost(
...
export default ImportBukuDigital;
