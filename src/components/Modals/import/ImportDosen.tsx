...
import { useImportDosen } from "@/services/dosen";
import { sampleDosenExport } from "@/services/dosen/http";
...
const ImportDosen = ({ isOpen, onClose, confirmAction }: Props) => {
  ...
  const {mutate: mutatePost} = useImportDosen();
  const handleSubmit = () => {
    setLoading(true)
    try {
      mutatePost(
      ...
      >
        <ModalContent>
          <ModalHeader className="flex items-center justify-between">
            <span className="font-semibold text-primary">Import Data Dosen</span>
          ...
          <ModalBody className="flex flex-col gap-y-5 p-8">
            <div>
              <Button onPress={handleDownloadTemplate} isLoading={isLoadingImport} className="bg-primary text-white w-full rounded-md"><BsCloudDownloadFill /> Sample Dosen Import</Button>
            </div>
            <div>
