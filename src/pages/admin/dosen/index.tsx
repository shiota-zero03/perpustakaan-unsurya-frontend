...
export default function DataDosen(){
    ...
    const [nidnSearch, setNidnSearch] = useState<string | null>(null);
    ...
    const DOSEN_DATA = useMemo(() => {
        if (!dataDosen || !dataDosen.data) return [];
        setTotalPage(dataDosen.data.pagination.totalPages || 0);
        setTotalData(dataDosen.data.pagination.totalItems || 0);
        setFromPage(dataDosen.data.pagination.from || 0);
        setToPage(dataDosen.data.pagination.to || 0);

        return dataDosen.data.data
    }, [dataDosen, currentPage]);
    ...
    const {mutate: mutateSelection} = usePostSelectedDosen();
    ...
    const {mutate: mutateDeleted} = useDeletedDosen();
                    ...
                    <div className="flex items-center sm:flex-row flex-col gap-2 w-full">
                        ...
                        <div className="w-full">
                            ...
                            <Input
                                ...
                                radius="sm"
                                value={nidnSearch || ""}
                                onChange={(e) => setNidnSearch(e.target.value)}
                                classNames={{
                                    inputWrapper: 'border border-primary',
                                    input: 'text-primary'
                                }}
